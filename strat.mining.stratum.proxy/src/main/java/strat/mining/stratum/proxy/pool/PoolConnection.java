...
public class PoolConnection extends StratumConnection {
	...
	public String getConnectionName() {
		return "PoolConfiguration-" + pool.getName();
	}
	...
	protected void onAuthorizeRequest(MiningAuthorizeRequest request) {
		LOGGER.warn("PoolConfiguration {} received an Authorize request. This should not happen.", pool.getName());
	}

	@Override
	protected void onSubscribeRequest(MiningSubscribeRequest request) {
		LOGGER.warn("PoolConfiguration {} received a Subscribe request. This should not happen.", pool.getName());
	}

	@Override
	protected void onSubmitRequest(MiningSubmitRequest request) {
		LOGGER.warn("PoolConfiguration {} received an Submit request. This should not happen.", pool.getName());
	}

	@Override
	protected void onExtranonceSubscribeRequest(MiningExtranonceSubscribeRequest request) {
		LOGGER.warn("PoolConfiguration {} received an Extranonce Subscribe request. This should not happen.", pool.getName());
	}

	@Override
	protected void onGetVersionRequest(ClientGetVersionRequest request) {
		LOGGER.debug("PoolConfiguration {} reply to GetVersion request.", pool.getName());
	...
	protected void onGetVersionResponse(ClientGetVersionRequest request, ClientGetVersionResponse response) {
		LOGGER.warn("PoolConfiguration {} received a GetVersion response. This should not happen.", pool.getName());
	}
}
